...
0. Outlook으로 이동하여 본인에게 다음과 같은 이메일을 발송한다.

    - 받는 사람: 현재 Copilot Studio에 로그인 되어 있는 본인 회사 이메일
    - 제목: `견적서 문의`
    - 내용:
...
```{note}
이 이메일은 이후 에이전트가 **메일 받기(V3)** 작업으로 조회할 테스트 데이터입니다. 실제 거래 메일 대신 본인에게 메일을 보내 건적서 생성 흐름을 안전하게 연습할 수 있도록 하기 위함입니다. 제목에 "견적서"가 포함되어야 뒤의 제목 필터 설정과 맞아떨어집니다.

해당 메일이 꼭 '받은 편지함'에 도착하는 지 확인 부탁드립니다. 개인 Outlook 설정 상 스스로에게 보낸 메일이 "내게 보낸 메일" 등의 별도의 메일함에 저장될 수도 있습니다. 이럴 땐 해당 메일을 '받은 편지함'으로 이동시켜야 이번 실습에서 만드는 프로토타입이 잘 작동합니다. 
``` 
...
```{note}
파일명에 `yyyymmddHHmmss` 형식의 타임스탬프를 붙이는 이유는, 에이전트를 실행할 때마다 날짜 및 시간이 파일명에 추가되게 하여, 고유한 파일명이 생성되도록 하여 기존 견적서가 덮어쓰여지는 것을 방지하기 위함과 동시에 직관적으로 언제 생성된 파일인지 확인하기 위함이다. 
```
...
```{note}
Excel 작업이 행을 추가할 대상 파일은 앞서 **경로를 사용하여 파일 복사**로 새로 만들어진 견적서 파일입니다. 이 파일을 지정하려면 복사 작업의 결과물에서 반환된 파일 Id가 필요하므로, AI가 이전 단계 output에서 해당 Id를 찾아 채우도록 설명을 입력하는 것입니다.
```
