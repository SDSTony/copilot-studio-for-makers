...
- 흩어져 있는 문서로부터 원하는 정보를 찾아주게끔 에이전트에게 기능을 추가하고자 합니다.
- 기존에는 문서를 하나씩 열어 Ctrl + F 등을 통해 검색을 했다면, 이번에 만드는 에이전트는 질문만 던지면 흩어져 있는 문서로부터 적절한 답변을 찾아서 정리해서 답변하기 때문에, 업무 효율화를 가져가 볼 수 있겠습니다.

## 지시사항

1. [Copilot Studio](https://copilotstudio.microsoft.com/)로 이동하여 본인의 개발 환경으로 이동한다.
...
3. **세부 정보를 편집** 버튼을 눌러 아래와 같이 편집하고 저장한다.
   ...
   - **이름**: 인사팀 신입 에이전트
   - **설명**: 인사팀의 행정적인 업무들을 보조합니다.

   ![세부 정보 편집](./imgs/lab04-01.png)

4. 에이전트의 **모델 선택** 드롭다운 메뉴를 사용하여, 모델을 **GPT-5 Chat**으로 변경한다.

   > ⚠️ **중요**: 현재 default 모델인 GPT-4.1은 성능이 최신 모델인 GPT-5에 비해 낮으므로, 실무에서도 항상 에이전트 모델 세팅을 비교적 최신 모델로 변경하여 쓸 수 있도록 하면 좋다.

   ![모델 변경](./imgs/lab04-02.png)

5. **참조 자료 추가**로 이동하여 **SharePoint**를 선택한 뒤, 이번 실습에 필요한 문서가 담긴 `Lab 04` 폴더를 추가한다.
...
14. 사용자 편의를 위해 추천 프롬프트에 아래와 같이 추가한다.
    ...
    > 참고: 추천 프롬프트는 2026년 2월 기준 테스트 패널에는 나타나지 않고, Teams 및 M365 Copilot에 추가된 에이전트로 이동했을 때 나타난다.
...
17. **Microsoft 365에서 에이전트 보기**를 클릭하면 M365 Copilot에 추가된 에이전트를 볼 수 있고, **Teams에서 에이전트 보기**를 클릭하면 Teams에 채팅 형태로 추가된 에이전트를 볼 수 있다. 게시 및 공유에 대한 개념은 교육 과정 후반부에서 더 자세히 살펴본다.

    > 참고: 2026년 2월 기준 게시된 에이전트가 Teams 및 M365 Copilot 채널에 추가가 가끔 안되는 경우가 있다. Microsoft 전체 플랫폼 단의 문제이고, Microsoft에서도 현재 조치 중이라고 한다. (PPAC 알려진 문제 ID: 6059816 ← PP 관리자라면 확인 가능)
