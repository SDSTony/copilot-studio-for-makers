# Lab 04 : 업무 맥락에 맞는 견적서 생성하는 에이전트 (Step-by-Step)
...
33. **도구** 탭에서 지금까지 추가된 모든 도구들을 확인할 수 있습니다:
    - 메일 받기(V3)
    - 경로를 사용하여 파일 복사
    - 테이블에 행 추가
...
34. **Excel Online(Business)** 커넥터를 연결한 뒤, "**테이블에 있는 행 나열**" (List rows present in a table) 작업을 선택하여 물품 단가표에서 데이터를 조회합니다.

    ![34. List rows present in a table 작업 선택](./imgs/new-quotation-agent/Slide34.JPG)

35. "List rows present in a table" 작업 설정에서 **연결** 필드가 설정되어 있고, **추가 및 구성**만 클릭하면 됩니다. 
...
37. **입력** 섹션에서:
    - **Location**: OneDrive for Business
    - **Document Library**: OneDrive (또는 문서)
    - **File**: 물품 단가표 파일 (예: "품목_컨설팅.xlsx")
    - **Table**: 물품 정보 테이블 선택
    
    **Table** 드롭다운에서 "**ItemPriceTable**"을 선택합니다. 이 테이블은 사내 물품들의 단가 정보를 포함하고 있습니다.

    ![38. 에이전트 모델 선택 및 설정](./imgs/new-quotation-agent/Slide38.JPG)


38. 에이전트 설정이 모두 완료되면, **구성** 탭의 "**에이전트의 모델 선택**" 섹션에서 **"GPT-5 Chat"**을 선택합니다. 이는 에이전트가 사용할 AI 모델입니다.

    ![37. ItemPriceTable 선택](./imgs/new-quotation-agent/Slide37.JPG)


39. 에이전트를 테스트하기 위해 **"사용해 보기"** 탭을 클릭합니다. 테스트 메시지를 입력하면 에이전트가 정의된 설정에 따라 도구들을 순서대로 실행합니다.

    ![39. 에이전트 테스트 시작 및 도구 실행](./imgs/new-quotation-agent/Slide39.JPG)

40. 테스트 실행 중 에이전트가 여러 도구들을 순차적으로 실행하는 과정을 확인할 수 있습니다. 각 도구의 실행 결과가 표시됩니다.

    ![40. 에이전트 도구 실행 진행 과정](./imgs/new-quotation-agent/Slide40.JPG)

41. 에이전트의 테스트 결과 목록에서 수행된 모든 작업들을 확인할 수 있습니다:
    - 메일 받기 완료
    - 테이블에 있는 행 나열
    - 테이블 행 추가 완료
    - 경로를 사용하여 파일 복사 완료
...
42. 최종적으로 생성된 견적서를 확인합니다. Excel 형식의 견적서에 다음 정보가 자동으로 입력되었습니다:
    - 회사명과 발행 날짜
    - 물품 항목별 단가 및 금액
    - 부가세 및 합계 금액
    
    이로써 이메일 요청 → 견적서 생성 → OneDrive 저장의 전체 워크플로우가 완성되었습니다.
