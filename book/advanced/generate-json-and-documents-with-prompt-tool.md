...
7. Copilot Studio 로 이동하여 에이전트 플로우 메뉴로 이동한다. 그리고 에이전트 흐름 버튼을 클릭한다.

![](imgs/lab08-06-agent-flow-menu.png)

8. 트리거 추가 메뉴에서 Microsoft Forms 커넥터를 선택한 뒤 '새 응답이 제출되는 경우' 트리거를 선택한다.

![](imgs/lab08-07-forms-trigger-select.png)

9. 양식 ID에서 제공하는 드롭다운 박스를 활용하여 앞서 만든 '코파일럿 교육 수료증 발급 설문'을 선택한다.

![](imgs/lab08-08-forms-trigger-config.png)

10. 트리거 하단에 프롬프트 실행 작업을 추가한다.

![](imgs/lab08-09-add-prompt-action.png)

11. 새 사용자 지정 프롬프트를 클릭한다.

12. 프롬프트 도구 제목을 '수료증 생성'으로 변경한 뒤, 출력은 '문서'로 변경한다.
...
14. 제공된 `certificate.docx` 파일을 업로드한다. 아래 이미지처럼 4개의 필드가 변수로 식별된 것을 확인할 수 있다.

![](imgs/lab08-12-certificate-template.png)

15. 프롬프트 도구의 모델을 GPT-5로 변경하고, 지침을 아래와 같이 입력해본다. 이 때, `/` 로 표시되어 있는 것들은 변수로 넣어야 하므로 '콘텐츠 추가' 버튼을 클릭하여 적절한 변수를 넣고 변수명을 설정한다. `date`만 Power Fx 변수로 넣고, 나머지는 텍스트 변수로 입력한다.
...
16. 각 변수에는 아래와 같이 샘플 데이터 또는 수식을 입력한다.
    - name: 홍길동
    - department: 영업
    - title: 코파일럿 심화
    - date: `Today()`

17. 테스트 버튼을 누르면 모델 응답에서 다운로드 할 수 있는 링크가 나타나는 것을 확인할 수 있다.

![](imgs/lab08-14-prompt-test-result.png)

18. 링크를 클릭해보면 수료증 워드 양식에 적절한 값들이 입력된 것을 확인할 수 있다.

![](imgs/lab08-15-certificate-preview.png)

19. 프롬프트 도구 저장을 클릭한다.

20. Forms에 제출된 결과물을 가져오기 위해 '새 응답이 제출되는 경우'와 '프롬프트 실행' 사이에 '응답 세부 정보 가져오기' 작업을 추가한다.

![](imgs/lab08-16-get-response-details.png)

21. 양식 ID는 앞서 만들었던 Forms 이름을 찾아서 선택하고, 응답 ID에는 '새 응답이 제출되는 경우'에서 반환되는 '응답 ID'를 선택한다.

![](imgs/lab08-17-response-details-config.png)

22. 프롬프트 실행에도 각 입력 파라미터에 적절한 값을 설정한다.

![](imgs/lab08-18-prompt-input-params.png)

23. 에이전트 흐름을 '초안 저장'한 뒤 '게시'를 클릭한다.

24. 테스트를 수동으로 실행한다.

![](imgs/lab08-19-manual-test.png)

25. 코파일럿 교육 수료증 발급 설문에 응답자로 접속하여, 본인 성함을 넣고, 소속과 교육명도 원하는 값을 선택해본다. 그리고 제출한다.

![](imgs/lab08-20-forms-submit.png)

26. 에이전트 흐름이 잘 실행되었는지 확인한다.

![](imgs/lab08-21-flow-run-result.png)

27. 에이전트 흐름의 제목을 '수료증 생성 및 메일 발송 플로우'로 변경한다.

![](imgs/lab08-22-flow-rename.png)

28. 프롬프트 실행에서 반환되는 워드 파일을 OneDrive에 저장하기 위해 OneDrive에 '2026 코파일럿 교육 심화' 폴더를 하나 만든다.

![](imgs/lab08-23-onedrive-folder.png)

29. 비즈니스용 OneDrive(OneDrive for Business) 커넥터에서 파일 만들기 작업을 프롬프트 실행 하단에 추가한다.

![](imgs/lab08-24-create-file-action.png)

30. 폴더 경로는 앞서 만든 '2026 코파일럿 교육 심화' 폴더를 선택한다.

31. 파일 이름은 아래 수식을 사용한다. 첫 번째 파라미터는 `cert_`, 두 번째 파라미터에는 [변경 해주세요] 를 삭제한뒤, 해당 위치에 Forms에 제출한 성함을 동적 콘텐츠로 입력한다. 세 번째 파라미터는 현재 시간을 뜻하며, 네 번째 파라미터는 파일 확장자명이다.

    ```
    concat('cert_', [변경 해주세요], '_', formatDateTime(utcNow(), 'yyyyMMddHHmmss'), '.docx')
    ```

![](imgs/lab08-25-filename-formula.png)

32. 파일 콘텐츠는 앞서 '프롬프트 실행' 단계에서 나온 결과물 중에 'Document Output Content Bytes'를 선택하여 입력한다.

![](imgs/lab08-26-file-content-bytes.png)

33. 수정된 흐름을 게시한 뒤 다시 테스트 한다. 이번에는 자동 테스트를 진행한다. 이 경우 앞서 수동 테스트를 통해 진행했던 샘플 데이터를 사용하게 된다.

![](imgs/lab08-27-auto-test.png)

34. 성공적으로 실행되었다면 OneDrive에 파일이 하나 만들어진 것을 확인할 수 있다.

![](imgs/lab08-28-onedrive-file-created.png)

35. 양식도 잘 채워진 것을 확인할 수 있다.

![](imgs/lab08-29-certificate-filled.png)

36. 해당 워드 문서를 PDF로 자동 변환하기 위해 'Word 문서를 PDF로 변환' 작업을 추가한다.

![](imgs/lab08-30-convert-pdf-action1.png)

![](imgs/lab08-31-convert-pdf-action2.png)

37. 변환된 PDF 콘텐츠를 파일로 저장하기 위해 OneDrive 커넥터의 파일 만들기 작업을 추가한다. 폴더 경로는 OneDrive 내에서 PDF를 저장하고자 하는 경로로 설정하고, 파일 이름과 파일 콘텐츠는 아래와 같이 설정한다.
...
38. 에이전트를 통해 완료 메시지를 받기 위해 Teams 커넥터의 '채팅 또는 채널에서 메시지 게시' 작업을 추가한다. Recipient는 본인의 이메일을 입력한다. 교육 담당자로써 수료증이 생성되면 알림을 받기 위함이다. 
...
40. 게시를 한 뒤 테스트를 하면 Teams에 추가된 에이전트로부터 알림이 잘 수신되는 것을 확인할 수 있다.

![](imgs/lab08-36-teams-notification.png)

```{note}
Outlook으로 메일 보내기 작업을 마지막에 추가한다면, 생성된 수료증 PDF 파일을 이메일에 첨부하여 수료증을 신청한 사용자에게 메일로 보내는 작업까지 자동화 할 수 있다.
```
